...
import { api_代理人赏金体现 } from "@/api/提现接口";
...
export default {
    ...
    actions: {
        ...
        开始提现({ state, dispatch, rootGetters }, [密码, 姓名]) {
            ...
            var 代理人分润资产 = rootGetters["agent/ShopBonus/dailiren_fenrun_zichan"];
            var 奖赏所有数据 = rootGetters['agent/奖赏/奖赏所有数据'];
            var 显示语 = [
                // type 0 不能提现 1 加载中 2 成功 3 失败 4 待审核
                {
                    名称: "促销分润",
                ...
                { 名称: "赏金", 金额: 奖赏所有数据.奖赏收益.balance, type: 0, 提示: "一元起提" },
            ];
            ...
            Promise.all([
                ...
                if (!x[0]) {
                    显示语[0].type = 3;
                    显示语[0].提示 = "网络错误";
                } else if(显示语[0].type!=0){
                    if (x[0].data.code == 200) {
                        var str = x[0].data.msg;
                        if (str.indexOf("审核") == -1) {
                            显示语[0].type = 2;
                            显示语[0].提示 = "成功";
                        } else {
                    ...
                    } else if (x[0].data.code == "PAYEE_USER_INFO_ERROR" || x[0].data.code == "PAYEE_ACC_OCUPIED") {
                        //需要输入名字
                        state.显示提现框 = false;
                        dispatch("红购使者/红购使者/设置姓名输入框", true, { root: true });
                    } else {
                        显示语[0].type = 3;
                        显示语[0].提示 = "失败";
                    }
                ...
                if (!x[1]) {
                    显示语[1].type = 3;
                    显示语[1].提示 = "网络错误";
                } else if(显示语[1].type!=0){
                    if (x[1].data && x[1].data.code == 200) {
                        var str = x[1].data.msg;
                        if (str.indexOf("审核") == -1) {
                            显示语[1].type = 2;
                            显示语[1].提示 = "成功";
                        } else {
                    ...
                    } else if (x[1].data && (x[1].data.code == "PAYEE_USER_INFO_ERROR" || x[1].data.code == "PAYEE_ACC_OCUPIED") ) {
                        //需要输入名字
                        state.显示提现框 = false;
                        dispatch("红购使者/红购使者/设置姓名输入框", true, { root: true });
                    } else {
                ...
                var 红购使者信息=rootGetters['红购使者/红购使者/红购使者信息'];
                console.log(红购使者信息)
                dispatch('agent/奖赏/奖赏初始化',红购使者信息,{root:true})
            ...
            //         显示语[0].type = 3;
            //         显示语[0].提示 = "失败";
            //         mui.alert(x.data.msg ? x.data.msg : x.data.message, "提示", "好的", function () { }, "div");
            //     }
            // });
        },
        店铺分润提现({ state, rootGetters }, 姓名) {
            ...
            return new Promise((resolve, reject) => {
                ...
                axios.post("/api-s/shops/AgentsShareProfit", obj).then(x => {
                    resolve(x);
                }).catch(err => {
                    resolve(false);
                });
        ...
        代理人赏金体现({state,rootGetters},姓名){
            ...
            var obj = {
                ...
                name: 姓名 ? 姓名 : "",
                account: 收款账号.account, //到账账号
                type: 3, //1代理人 2 代理商 3 分润提现
                amount: 奖赏所有数据.奖赏收益.balance,
                identity: userInfo.username + "_" + new Date().getTime()
            };
            return new Promise((resolve, reject) => {
                if (!obj.amount || obj.amount < 1) {
