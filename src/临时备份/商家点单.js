            ...
            list.forEach(item=>{
                ...
                if(item.可享受新人专享 && 商品index<=专享最大数量 && item.专享商品){
                    ...
                    if(商品红包){
                        ...
                        let 红包1 = Object.assign({},用户新人红包);
                            红包1.实际抵扣 = 新人红包金额<商品可抵扣 ? 新人红包金额 : 商品可抵扣;
                        item.使用红包.push(红包1);
                        新人红包金额=新人红包金额 - 商品可抵扣;
                    }else if(平台红包金额>0){
                        let 红包1 = Object.assign({},state.用户平台红包);
                            红包1.实际抵扣 = 平台红包金额<商品可抵扣 ? 平台红包金额 : 商品可抵扣;
                        item.使用红包.push(红包1);
                        平台红包金额=平台红包金额 - 商品可抵扣;
                    }else if(节日红包金额>0){
            ...
            if(ApplicationType=='app'){
                function 跳转地图(){
                    if(AppType=='ios'){
                        ...
                        // var dst = new plus.maps.Point(state.当前位置.x , state.当前位置.y);
                        // var src = new plus.maps.Point(state.店铺信息.x , state.店铺信息.y);
                        // 调用系统地图显示 
