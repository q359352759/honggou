<template>
    <div>
        <header class="mui-bar mui-bar-nav">
            ...
            <h1 class="mui-title">商品管理2</h1>
            <span @click="显示排序=!显示排序" class="add mui-pull-right">{{显示排序 ? '完成' :"排序"}}</span>
        </header>
            ...
            <ul class="商品容器">
                <li class="分组列表">
                    <div :class="{'active':当前查看分组==index}" v-for="(item, index) in 商品分组" :key="index">
                        ...
                        <span v-show="显示排序" class="排序"><i class="icon iconfont icon-shang"></i></span>
                    </div>
                </li>
                <li class="商品区" @scroll="scroll($event)" ref="商品区">
                    <div ref="去除高度"></div>
                    <div v-show="查看类型==1" ref="上架容器">
                        <div v-for="(item_1, index) in 已上架" :key="index">
                            <ul class="商品" v-for="(item, index) in item_1" :key="index">
                                ...
                                <div v-show="显示排序" class="排序"><i class="icon iconfont icon-shang"></i></div>
                            </ul>
                        </div>
                    </div>

                    <div v-show="查看类型==0" ref="下架容器">
                        <div v-for="(item_1, index) in 已下架" :key="index">
                            <ul class="商品" v-for="(item, index) in item_1" :key="index">
                                <li @click="go_addGoods(item)">
                                    <img v-if="item.img.split(',').length>0" :src="item.img.split(',')[0]" />
                                    <span>编辑</span>
...
import { openloading , limit } from "@/assets/js/currency";
                    ...
                    if (value.index == 1) {
                        ...
                        Promise.all([this.修改商品([obj]), this.删除专享(x.EXID)]).then(res => {
                            if(res[0].data.code == 200){
                                ...
                                //下线了从上线列表中移除
                                if(x.state==0){
                                    this.已上架商品.commodity_list.splice(index,1);
                                    this.已上架商品.total--
                                    this.已下架商品.commodity_list.push(XMLHttpRequest);
                                    this.已下架商品.total++
                                }else{
                                    this.已下架商品.commodity_list.splice(index,1)
                                    this.已下架商品.total--
                                    this.已上架商品.commodity_list.push(x);
                                    this.已上架商品.total++
                                }
                            }
                ...
                api_修改商品([obj]).then(res => {
                    if (res.data.code == 200) {
                        ...
                        if(x.state==0){
                            this.已上架商品.commodity_list.splice(index,1)
                            this.已上架商品.total--
                            this.已下架商品.commodity_list.push(x)
                            this.已下架商品.total++
                        }else{
                            this.已下架商品.commodity_list.splice(index,1)
                            this.已下架商品.total--
