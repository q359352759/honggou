<template>
    <div>
        <header class="mui-bar mui-bar-nav">
            ...
            <h1 class="mui-title">评价详情1</h1>
        </header>
        <div class="mui-content mui-fullscreen">
            <div class="box_1" v-for="(item, index) in 评论.list" :key="index">
                <!-- 购买商品 -->
                <ul class="商品信息" v-if="订单.orderType==0">
                    <li class="商品图片">
                ...
                <!-- 输入金额购买 -->
                <ul class="店铺信息" v-if="订单.orderType==1">
                    <li class="icon_box">
...
<script>
import { api_添加评论 , api_修改评论 ,api_根据id查询评论 , api_查询评论列表 } from "@/api/评论接口.js";
    ...
    methods: {
        ...
        async 回复评论(){
            ...
            if(!feikong.test(remark_text)){
                ...
                remark=b64EncodeUnicode(remark) //转码
            var 评价对象={
                    id:"",          //自增id
                    ordersid:'',    //订单id
                    commodityid: '', //商品id
        ...
        更新回复(){
            var this_1=this;
            api_根据id查询评论(this.回复对象.id).then(x=>{
                if(x.data.code==200){
                    var 新回复=x.data.data;
        ...
        async 初始化(){
            ...
            Promise.all([this.根据订单id查询(this.订单自增Id),api_查询评论列表(this.query)]).then(x=>{
                var 订单返回值=x[0];
                   this.订单=订单返回值.data.data;                    
                var 购买的商品=this.订单.shopOrderCommoditys

                api_根据shopid查询店铺(this.订单.shopid).then(x=>{
                    this.店铺=x.data.code==200 ? x.data.data : '';
                })
