        ...
        <div class="mui-content mui-fullscreen" @scroll="scroll($event)">
            ...
            <ul class="box_2" v-if="type==0">
                <li v-for="(x, index) in commodity_list.list" :key="index" :class="{'pos_left':x.pos_left,'pos_right':x.pos_right,'active':x.active}">
                    ...
                    <div v-on:touchend="touchend($event,x)" v-on:touchstart="huoqu($event,x)" v-on:touchmove="touchmove($event)">
                        <ul class="cont_1">
                            <li @click="$router.push('/commodity/CommodityDetails?id=' + x.commodityId + '&isshop=1')">
                            ...
                            <li>
                                <h1>
                                    ...
                                    <div @click="$router.push('/commodity/CommodityDetails?id=' + x.commodityId + '&isshop=1')">
                                        {{x.commodityName}}
                                    </div>
                                </h1>
                                <h2 @click="$router.push('/commodity/CommodityDetails?id=' + x.commodityId + '&isshop=1')">
                                    <span class="mui-pull-right">人气：{{x.商品 ? x.商品.popularity : 0}}</span>
                                    价格：{{x.sellingPrice}}元
                                </h2>
                                <h3 @click="$router.push('/commodity/CommodityDetails?id=' + x.commodityId + '&isshop=1')">红包省：{{x.deduction}}元</h3>
                            </li>
                            <!--<li>
            ...
            <ul class="box_3" v-if="type==1">
                <li v-for="(x, index) in shop_list.list" :key="index" :class="{'pos_left':x.pos_left,'pos_right':x.pos_right,'active':x.active}">
                    ...
                    <div v-on:touchend="touchend($event,x)" v-on:touchstart="huoqu($event,x)" v-on:touchmove="touchmove($event)">
                        <ul class="cont_1">
                            <li @click="$router.push('/sjzst2?shopid=' + x.shopid)">
                                <img  :src="x.signboard" alt="" srcset="">
                            </li>
                            <li>
                                <h1>
                                    <div @click="Recommend()" class="mui-pull-right"><i class="icon iconfont icon-fenxiang2"></i></div>
                                    <div @click="$router.push('/sjzst2?shopid=' + x.shopid)">{{x.shopName}}</div>
                                </h1>
                                <h2 @click="$router.push('/sjzst2?shopid=' + x.shopid)">
                                    <span class="mui-pull-right">人气：{{x.店铺 ? x.店铺.popularity : 0}}</span>
                                    <div> 地址：{{x.address}}</div>
                                </h2>
                                <h3 @click="$router.push('/sjzst2?shopid=' + x.shopid)">{{(x.店铺评分 && x.店铺评分.scoreAvg ) ? x.店铺评分.scoreAvg : 5}}分</h3>
                            </li>
                            <!-- <li>
                                <div @click="Recommend()"><i class="icon iconfont icon-fenxiang2"></i></div>
                                <span>人气：666</span>
...
import { api_根据shopid查询店铺 , api_查询店铺评分 } from "@/api/店铺接口.js";
...
export default {
    ...
    methods: {
        ...
        go_shop(x) {
            // http://192.168.1.13:8080/#/sjzst2?shopid=D7004090906D139CD1492008D376E457
                    ...
                    if (x.data.code == 200) {
                        ...
                        list.forEach(item => {
                            ...
                            if(item.shopid){
                                ...
                                api_查询店铺评分(item.shopid).then(r=>{
                                    item.店铺评分=(r.data.code==200 && r.data.data.length>0) ? r.data.data[0] : ''; 
                                }).catch(err=>{})
                            }
                        });
