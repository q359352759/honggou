            ...
            <div class="conent_1" @scroll="scroll($event)">
                ...
                <ul class="Red_envelopes" v-if="type==1">
                    <li v-for="(item, index) in invitedrecord.list" :key="index">
                        <div class="header_1">
                            <span class="time_1 mui-pull-right">{{item.createtime | filter_time("yyyy.MM.dd")}}</span>
                ...
                <ul class="Red_envelopes" v-show="type==2">
                    <li v-for="(item, index) in CardPackge_0.list" :key="index">
                        <div class="header_1">
                            <span class="time_1 mui-pull-right">{{item.pgCreateTime | filter_time("yyyy.MM.dd")}}</span>
                        ...
                        <ul class="item">
                            ...
                            <li>
                                ...
                                <div class="title_2">
                                    <span v-if="item.redDeductionType==1">每满{{item.redExpire}}元，可省{{item.redDeduction}}元</span>
                                    <span v-if="item.redDeductionType==0">省{{item.redPercentage}}%</span>
                                </div>
                                <div class="time_2">
                            ...
                            <li>
                                ...
                                <div v-if="item.redType==5">
                                    ...
                                    <span v-if="item.state==1" class="time_1">{{item.pgUpdateTime | filter_time("yyyy.MM.dd")}}</span>
                                ...
                                <div v-if="item.redType==0">
                                    ...
                                    <div v-if="item.state==1" class="type_2">已使用</div>
                                    <span v-if="item.state==1" class="time_1">{{item.updateTime | filter_time("yyyy.MM.dd")}}</span>
                                </div>
                                <!-- 有时间限制类型的 -->
                ...
                <ul class="Red_envelopes" v-show="type==3">
                    <li v-for="(item, index) in CardPackge_1.list" :key="index">
                        <div class="header_1">
                            ...
                            <span class="title_1" @tap="go_shop(item)">{{item.shopName ? item.shopName : '店铺名未知'}}</span>
                        </div>
                        <ul class="item">
                            <li class="img_box">
...
export default {
    ...
    methods: {
        ...
        //店铺红包  跳转店铺
        go_shop_1(item) {
            this.$router.push("/sjzst2?shopid=" + item.sShopid);
        },
        //商品红包  跳转商店
        go_shop(item) {
            this.$router.push("/sjzst2?shopid=" + item.redShopid);
        },
        //商品红包 跳转到商品
