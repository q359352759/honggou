    ...
    methods: {
        ...
        ApplicationShop() {
            if(this.userInfo){
                ...
                if(this.pid){
                    this.$router.push("/shenqingkaidian?pid="+this.pid);
                }else{
                    this.$router.push("/shenqingkaidian");
                }
            }else{
                this.设置是否显示登录框(true);
                // var html="<div style='text-align:center;'>你还未注册或登录</div>"+
                //             "<div style='text-align:center;'>请先注册或登录</div>"
                // mui.confirm(html, "提示", ["取消", "是的"], value => {
                //     if (value.index == 1) {
        ...
        init() {
            ...
            } else if (this.myshop.shopid && this.myshop.state == 2) {
                console.log("有店铺没通过");
                this.get_myshop = false;
                this.$router.push("/shenqingkaidian");
            } else if (this.myshop.shopid && (this.myshop.state == 1 || this.myshop.state == 3) ) {
                console.log("有店铺");
                // this.get_myshop=false
