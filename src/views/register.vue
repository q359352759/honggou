            ...
            <form class="box_1" @submit.prevent="submit()">
                <ul>
                    <li>
                        ...
                        <input type="text" @blur="失去焦点()" v-model="phone" required pattern="^1\d{10}$">
                        <!-- oninvalid="setCustomValidity('无效号码');" oninput="setCustomValidity('') -->
                        <span v-if="!phone" class="tishi_1">手机号码</span>
                    </li>
                    <li>
                        <i class="icon iconfont icon-yanzhengma"></i>
                        <input type="text" @blur="失去焦点()" v-model="code" required />
                    ...
                    <li>
                        ...
                        <input type="password" @blur="失去焦点()" v-model="password" required>
                        <span v-if="!password" class="tishi">（6-16位数字字母组成）</span>
                        <span v-if="!password" class="tishi_1">登录密码</span>
                    </li>
                    <li>
                        <i class="icon iconfont icon-san"></i>
                        <input type="password" @blur="失去焦点()" v-model="password1" required>
    ...
    methods: {
        ...
        get_phone() {
            ...
            return new Promise((resolve, reject) => {
                this.$axios
                    .get("/api-u/users/findByPhone?phone=" + this.phone)
                    .then(x => {
                        openloading(false);
                        if (x.data.code == 200) {
