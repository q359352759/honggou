            ...
            <div class="mui-col-xs-9 right" ref="right" @scroll="scroll($event)">
                ...
                <div class="box_3" v-for="(item, index) in type_list" :key="index">
                    <div class="title">
                        ...
                        {{item.name}}
                        <span @click="$router.push('/yewushichang/dianpufenlei?id='+item.id+'&name='+item.name)" class="mui-pull-right">去频道 <i class="mui-icon mui-icon-forward"></i></span>
                    </div>
                    <ul class="list mui-row">
                        <li class="mui-col-xs-4" v-for="(x, index) in item.children" :key="index">
                            <span @click="$router.push('/yewushichang/dianpufenlei?id='+x.id+'&name='+x.name)">{{x.name}}</span>
                        </li>
                    </ul>
                </div>
    ...
    watch: {
        list() {
            this.$nextTick(function () {
                ...
                var dom=this.$refs.right.childNodes
                dom.forEach(x => {
                    if (x.offsetTop)
