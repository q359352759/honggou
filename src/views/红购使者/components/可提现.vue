<template>
    <div>
        <ul class="账户" @click="跳转设置收款账号()">
            <li>
                <span>收款账户：</span>
                <span v-if="收款账号.type==0">支付宝</span>
                <span v-if="收款账号.account && 实名信息 && 实名信息.name">(*{{实名信息.name.substring(1)}})</span>
                <span>
                    {{收款账号.account | 过滤账号}}
                </span>
            </li>
...
export default {
    ...
    computed: {
        ...mapGetters({
            ...
            收款账号: "红购使者/红购使者/收款账号",
            代理人分润资产: "红购使者/红购使者/代理人分润资产", //代理人分润资产
            实名信息: "红购使者/红购使者/实名信息",
    ...
    methods: {
        ...
        跳转设置收款账号(){
            if(this.当前登录用户.username==this.userid){
                ...
                mui.toast("你不能修改收款账号。", { duration: "long", type: "div" });
            }
        },
        提现() {
            if (!this.同意协议) {
