            ...
            <div class="box_1">
                <ul class="mui-table-view">
                    <li class="mui-table-view-cell item">
                        ...
                        <input type="text" placeholder="实名认证后自动填入" v-model="申请对象.realName" readonly/>
                    </li>
...
export default {
    ...
    mounted() {
        ...
        //获取实名信息
        this.$axios
            .get("/api-u/certification/findByUserid?userid=" + this.userInfo.username)
            .then(x => {
                console.log(x);
                if (x.data.code==200 && x.data.data ) {
                    this.实名认证信息 = x.data.data;
                    this.申请对象.realName = x.data.data.name;
                }
            })
            .catch(err => { });
    }
