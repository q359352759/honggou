    ...
    methods: {
        ...
        Sure() {
            if (this.size == 0) {
                ...
                //判读是否有图片url
                if (this.Select_picture.length > 0 && typeof this.Select_picture[0] == "string") {
                    var list = [];
                    var Select_picture = this.Select_picture;
                    for (var i = 0; i < Select_picture.length; i++) {
                        list.push({ url: Select_picture[i] });
                    }
                    this.$store.state.Select_picture.list = list;
                } else {
                    this.$store.state.Select_picture.list = this.Select_picture;
                }
                history.back();
            } else {
                mui.toast("最多选择" + this.size + "张。", { duration: "long", type: "div" });
            }
        },
        //选择
        select_img(x) {
        ...
        add_phone() {
            ...
            this.$axios({
                ...
                if (x.data.code == 200) {
                    ...
                    this.img_list.unshift(x.data.data[0]);
                } else {
