    ...
    methods: {
        ...
        goumai() {
            ...
            if(ApplicationType=='app'){
                this.$axios.post('/api-u/users/zhifubao/guanggaoji',this.$qs.stringify({amount:498})).then(x=>{
                    console.log(x);
                    if(x.data.code==200){
        ...
        addguanggaoji() {
            var this_1 = this;
            var tishi =
                "恭喜您购买广告机成功，点击公众号内的“广告管理”使用，初始账号为您的手机号，密码为手机号后6位，请在广告机内点击“会员服务”修改密码（请勿在红包乐购中修改）。";
        ...
        get_angentActivation() {
            ...
            this.$axios
                .post(
                    "http://www.lxad.vip/api/get_angentActivation.php",
        ...
        use() {
            ...
            if (!this.jihuo_obj.phone || !phone_test.test(this.jihuo_obj.phone)) {
                ...
                !this.jihuo_obj.pwd ||
                this.jihuo_obj.pwd.length < 6 ||
                this.jihuo_obj.pwd.length > 12
            ) {
            ...
            this.$axios
                .post(
                    "http://www.lxad.vip/api/use_angentActivation.php",
                    this.$qs.stringify(this.jihuo_obj)
                )
                .then(x => {
                    console.log(x);
                    openloading(false);
                    var tishi =
                        "恭喜您激活成功，点击公众号内的“广告管理”使用，请在广告机内点击“会员服务”修改密码（请勿在红包乐购中修改）。";
