    ...
    <div id="LegalPersonCertification">
        ...
        <div class="mui-content">
            <div v-if="userinfo.iaiState" class="box_3">
                <ul class="mui-table-view">
                    <li class="mui-table-view-cell item">
                        <div @click="select_shengfen(true)" class="radio_1" :class="{'active':法人证件选择类型==0}">
            ...
            <div v-if="userinfo.iaiState" class="box_3">
                <ul class="mui-table-view">
                    <li class="mui-table-view-cell item">
                        <div class="radio_1" :class="{'active':法人证件选择类型==1}">
                            <i class="icon iconfont icon-xuanze"></i>
                        </div>
                        <div>使用营业执照法人认证</div>
                    </li>
...
export default {
    ...
    methods: {
        ...
        select_shengfen() {
            ...
            this.更新店铺key(["iaiAddress", this.findByUserid.address]);
        ...
        add() {
            ...
            try {
                console.log('try未发现错误')
                this.更新店铺key(["frontImg", this.zhengmian_img]);
            } catch (error) {
                openloading(false)
                this.更新店铺key(["frontImg", ""]);
                mui.alert('应用内存不足，无法写入，请删除部分图片或选择较小的图片或退出再试'+error, "提示", "我知道了", function () { }, "div");
                return
            }
            try {
                console.log('try未发现错误')
    ...
    mounted: function () {
        ...
        if(this.法人证件选择类型==1){
            this.Positive_obj.sex=this.店铺.sex=="男" ? 0 : 1;
            this.Positive_obj.nation=this.店铺.nation;
            this.Positive_obj.birthday=this.店铺.birthday
            this.Positive_obj.address=this.店铺.iaiAddress;
            this.Positive_obj.idNumber=this.店铺.idNumber;
            this.Positive_obj.name=this.店铺.iaiName
            if(this.Positive_obj.nation && this.Positive_obj.birthday && this.Positive_obj.address && this.Positive_obj.idNumber && this.Positive_obj.name){
                this.zhengmian_ok=true;
            }
            this.zhengmian_img=this.店铺.frontImg;
            this.fanmian_img=this.店铺.reverseImg;
            this.The_other_side.issueArea=this.店铺.issueArea
            this.The_other_side.Invalid=this.店铺.validity;
            if(this.The_other_side.issueArea && this.The_other_side.Invalid){
                this.fanmian_ok=true;
            }
        }
