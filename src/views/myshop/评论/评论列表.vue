    ...
    <div>
        ...
        <div class="mui-content mui-fullscreen" @scroll="scroll($event)">
            ...
            <div class="box_2" v-show="huifu_box">
                ...
                <ul class="content_1">
                    <li class="name" v-if="回复对象.用户">回复：{{回复对象.用户.nickname | 名字转码}}</li>
...
<script>
import { api_添加评论 , api_修改评论 , api_根据id查询评论 } from "@/api/评论接口.js";
...
export default {
    ...
    methods: {
        ...
        async 回复评论(){
            ...
            remark=b64EncodeUnicode(remark);
            ...
            api_根据id查询评论(this.回复对象.id).then(x=>{
                if(x.data.code==200){
                    var 新回复=x.data.data;
                        新回复.用户={};
                        try {
                            新回复.remark = b64DecodeUnicode(新回复.remark)
                        } catch (error) {}
    ...
    mounted() {
        ...
        this.顾客id=this.$route.query.kehu ? this.$route.query.kehu :""
        this.初始化();
