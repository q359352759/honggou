    ...
    <div>
        ...
        <div class="mui-content mui-fullscreen">
            ...
            <div class="内容区" @scroll="滚动($event)">
                <ul class="列表" >
                    <li v-for="(item, index) in 顾客.list" :key="index">
                        <div>{{item.createTime | filter_time}}</div>
                        <div>
                            <span v-if="item.USERNAME">{{item.USERNAME | filter_name}}</span>
                            <span v-if="!item.USERNAME">--</span>
                        </div>
                        <div>
                            <span class="蓝色" v-if="item.REALNAME">{{item.REALNAME | filter_name}}</span>
                            <span v-if="!item.REALNAME">--</span>
                        </div>
                        <div class="蓝色" @click="$router.push('/myshop/Member/xiaofeijilu?userid='+item.userid)">{{item.PAYMENTAMOUNT}}</div>
                        <div @click="跳转评论(item)" class="蓝色">{{item.评论条数}}条</div>
                    </li>
                </ul>
...
export default {
    ...
    methods: {
        ...
        跳转评论(item){
            if(item.评论条数!=0){
                this.$router.push('/myshop/comment/commentList?kehu='+item.userid);
            }else{
                mui.toast("暂无评论", { duration: "long", type: "div" });
            }
        },
        getSort(x) {
            console.log(x);
            ...
            this.$axios.get("/api-s/shops/findShopCustomerAll", { params: this.顾客.query }).then(x => {
                    ...
                    if (x.data.code == 200) {
                        ...
                        list.forEach(item => {
                            ...
                            api_查询评论列表(查询评论).then(r=>{
                                item.评论条数=r.data.code==200 ? r.data.data.total : 0;
                            })
                        });
                        this.顾客.list = this.顾客.list.concat(list);
