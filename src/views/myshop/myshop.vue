    ...
    <div id="myshop">
        ...
        <div class="mui-content mui-fullscreen">
            ...
            <ul class="box_2">
                ...
                <li>
                    <div @click="跳转点单()">
                        <i class="icon iconfont icon-diandan"></i>
                        <span class="角标" v-show="待制作点单数量!=0">{{待制作点单数量}}</span>
                    </div>
                    <div>点单</div>
                </li>
                <li>
                    <div @click="$router.push('/myshop/dth')">
                        <i class="icon iconfont icon-tihuoren"></i>
                        <span class="角标" v-show="待取货数量!=0">{{待取货数量}}</span>
                    </div>
                    <div>提货</div>
                </li>
                <li>
                    <div @click="$router.push('/myshop/yd/ydlb')">
                        <i class="icon iconfont icon-ziyuan-copy"></i>
                        <span v-if="预定待消费数量>0" class="角标">{{预定待消费数量>99 ? '99+' : 预定待消费数量}}</span>
                    </div>
                    <div>预定</div>
                </li>
                <li>
                    <div @click="$router.push('/myshop/ps/pslb')">
                        <i class="icon iconfont icon-Icon"></i>
                        <!-- <span class="角标">-0</span> -->
                    </div>
                    <div>配送</div>
                </li>
                <li v-if="店铺身份==1">
            ...
            <ul class="box_4">
                ...
                <li @click="$router.push('/myshop/js/tgjs')">
                    <div class="标题 标题1">奖赏</div>
                    <div class="红色字体">推广：{{奖赏数量}}件</div>
                </li>
                <li @click="跳转提现()">
                    <div class="标题 标题1">提现</div>
                    <div>可提现{{ketixian}}元</div>
                </li>
            </ul>
            ...
            <ul class="半圆列表">
                <li>
                    <div>
                        <div @click="$router.push('/Notice')">店铺公告</div>
                    </div>
                    <div>
                        <div @click="$router.push('/myshop/jieshao')">店铺介绍</div>
                    </div>
                    <div>
                        <div @click="$router.push('/albumManagement')">相册管理</div>
                    </div>
                </li>
                <li>
                    <div>
                        <div @click="进入店铺()">店铺预览</div>
                    </div>
                    <div>
                        <div @click="跳转红购返利()">红利返现</div>
                    </div>
                    <div>
                        <div>必&nbsp;抢</div>
                    </div>
...
import { api_查询排号 } from "@/api/点单接口.js";
...
export default {
    ...
    methods: {
        ...
        查询待制作点单(){
            ...
            api_查询排号(查询待制作_query).then(x=>{
                this.待制作点单数量=x.data.code==200 ? x.data.data.total : 0;
            })
        },
        查询推广(){
            var query={
            ...
            this.奖赏数量=0;
            api_查询奖赏列表(query).then(x=>{
                console.log('奖赏列表',x);
                if(x.data.code==200){
                    var list=x.data.data.data;
                    list.forEach(item => {
                        this.奖赏数量+=item.havarewardnum;
                    });
                }
            }).catch(err=>{})
        },
        ...
        async shopinit() {
            ...
            api_查询订单列表(预定待消费_query).then(x=>{
                this.预定待消费数量=x.data.code==200 ? x.data.data.total : 0
            })
