    ...
    <div id="ApplicationShop">
        ...
        <div class="mui-content mui-fullscreen">
            ...
            <ul class="mui-table-view box_1">
                ...
                <li class="mui-table-view-cell input_box">
                    <span>微信客服：</span>
                    <input type="text" v-model="shop_obj.wechat" placeholder="请输入客服的微信号">
                </li>
                ...
                <li class="mui-table-view-cell" @click="$router.push('/baiduMap')">
                    <a class="mui-navigate-right">
                        <span>店铺地址：</span>
                        {{address}}
                    </a>
                ...
                <li class="mui-table-view-cell input_box">
                    <span>详细位置：</span>
                    <input type="text" placeholder="如单元、楼层、门牌号" v-model="shop_obj.remark">
                </li>
                ...
                <li class="mui-table-view-cell" @click="$router.push('/shopAuthentication')">
                    <a class="mui-navigate-right">
                        <span>法人认证：</span>
                        {{this.$store.state.apply_for_a_shop.iaiName}}
                    </a>
                </li>
                <li class="mui-table-view-cell" @click="$router.push('/shopBusinessDetails')">
                    <a class="mui-navigate-right">
                        <span>营业执照：</span>
...
<script>
import { api_修改店铺 , api_添加店铺修改表 , api_根据userid查询修改表数据} from '@/api/店铺接口.js'
    ...
    methods: {
        ...
        查询店铺修改情况() {
            new Promise((resolve, reject) => {
                api_根据userid查询修改表数据(this.myshop.userid).then(x=>{
                    if (x.data.code == 200) {
                        if (x.data.data) {
                            this.isUpdata = false;
                        } else {
        ...
        async add() {
            ...
            if (!this.shop_obj.shopType) {
                ...
                return;
            } else if (!phone_test.test(this.shop_obj.phone) && !zuoji_test.test(this.shop_obj.phone)) {
                mui.toast("电话号码填写有误", { duration: 2000, type: "div" });
                return;
