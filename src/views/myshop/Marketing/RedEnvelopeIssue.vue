...
export default {
  ...
  methods: {
    ...
    wenhao() {
      ...
      if (this.type == 0) {
        html =
          '<ul style="text-align:left;color:#777777">' +
          '<li style="text-indent: 20px;">红包金额不管设置多大，都按照红包使用规则进行省钱。</li>' +
          "<li>如：发放800元的红包，省方式为“可省支付金额的10%”。</li>" +
          '<li style="text-indent: 20px;margin:0px 0px 10px;">1、顾客输入金额，支付100元，可用红包省10元。顾客还剩余790元的红包，可以下次购买时使用。</li>' +
          '<li style="text-indent: 20px;margin:0px 0px 10px;">2、顾客购买商品，该商品设置的可省金额为20元，那么顾客购买时可以用红包省20元。顾客还剩余780元的红包，可以下次购买时使用。</li>' +
        ...
        html =
          '<div style="color:#777777">红包金额为商品的可省金额，不可修改</div>';
      } else if (this.type == 2) {
        html =
          '<div style="color:#777777">根据促销商品的可省金额来计算，建议红包金额大一些，对消费者更有吸引力！红包再大依然遵循可省金额，带来的营业额也更多。</div>';
      } else if (this.type == 4) {
        html =
          "<div style='color:#777777'>根据促销商品的可省金额来计算，建议红包金额大一些，对消费者更有吸引力！红包再大依然遵循可省金额，带来的营业额也更多。</div>";
      } else if (this.type == 5) {
        html =
          "<div style='color:#777777'>如果有发布生日专享商品，根据生日专享商品可省金额计算，否则根据几款普通商品的可省金额计算，建议红包大一些。红包再打大然遵循可省金额，但能购给消费者留下较好印象。</div>";
      }
    ...
    sub() {
      ...
      try {
        this.Red_envelopes.amount = this.Red_envelopes.amount.replace(
          /\s+/g,
          ""
        );
      } catch (error) {}
      ...
      if (this.type == 0) {
        ...
        if (!this.Red_envelopes.headline) {
          ...
          !this.Red_envelopes.startTime ||
          !this.Red_envelopes.endTime ||
          this.Red_envelopes.startTime >= this.Red_envelopes.endTime
        ) {
          mui.toast("使用期限有误。", { duration: 2000, type: "div" });
          return;
        } else if (
          this.Red_envelopes.startTime <
          getDateStr(-30, this.Red_envelopes.endTime)
        ) {
          mui.toast(
            "<span style='white-space: nowrap;'>使用时间最大为30天。</span>",
          ...
          !zhengshu_test.test(this.Red_envelopes.quantityMax_1) ||
          this.Red_envelopes.quantityMax_1 < 1
        ) {
        ...
        if (!this.Red_envelopes.headline) {
          ...
          !this.Red_envelopes.signInStartTime ||
          !this.Red_envelopes.signInEndTime ||
          this.Red_envelopes.signInStartTim >= this.Red_envelopes.signInEndTime
        ) {
          mui.toast("签到时间有误。", { duration: 2000, type: "div" });
          return;
        } else if (
          this.Red_envelopes.signInStartTime <
          getDateStr(-15, this.Red_envelopes.signInEndTime)
        ) {
          mui.toast(
            "<span style='white-space: nowrap;'>签到时间最大为15天。</span>",
            { duration: 2000, type: "div" }
          );
          return;
        } else if (
          ...
          this.Red_envelopes.startTime <
          getDateStr(-15, this.Red_envelopes.endTime)
        ) {
          mui.toast(
            "<span style='white-space: nowrap;'>使用时间最大为15天。</span>",
            { duration: 2000, type: "div" }
          );
          return;
        } else if (
          !number_test.test(this.Red_envelopes.startAmount) ||
          this.Red_envelopes.startAmount < 0
        ) {
          mui.toast("起始金额有误。", { duration: 2000, type: "div" });
          return;
        } else if (
          !number_test.test(this.Red_envelopes.progressivelyAmount) ||
          this.Red_envelopes.progressivelyAmount < 0
        ) {
          mui.toast("递增金额有误。", { duration: 2000, type: "div" });
          return;
        } else if (
          !zhengshu_test.test(this.Red_envelopes.increaseDays) ||
          this.Red_envelopes.increaseDays < 0
        ) {
          mui.toast("递增天数有误。", { duration: 2000, type: "div" });
          return;
        } else if (
          !zhengshu_test.test(this.Red_envelopes.cycleDays) ||
          this.Red_envelopes.cycleDays < 0
        ) {
          mui.toast("循环天数有误。", { duration: 2000, type: "div" });
          return;
        } else if (
          this.Red_envelopes.increaseDays > 15 ||
          this.Red_envelopes.cycleDays > 15
        ) {
          mui.toast("递增天数和循环天数不能大于15天。", {
            duration: 2000,
            type: "div"
          });
          return;
        } else if (
          this.Red_envelopes.increaseDays > this.Red_envelopes.cycleDays
        ) {
          mui.toast("递增天数不能大于循环天数。", {
            duration: 2000,
            type: "div"
          });
          return;
                  ...
                  value => {
                    if (value.index == 1) {
                      ...
                      if (x.data.data.data[0].quantityMax) {
                        this.Red_envelopes.quantityMax =
                          this.Red_envelopes.quantityMax_1 +
                          x.data.data.data[0].quantityMax;
                      }
                      this.Submission();
                    }
                  },
        ...
        .then(x => {
          ...
          if (x.data.code == 200) {
            ...
            if (data.signInStartTime) {
              data.signInStartTime = dateFtt(
                data.signInStartTime,
                "yyyy-MM-dd"
              );
            }
  ...
  mounted: function() {
    ...
    console.log(
      "2018-10-01" < getDateStr(-30, "2018-10-31"),
      getDateStr(-30, "2018-10-31")
    );
    var query = this.$route.query;
