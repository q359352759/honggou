<template>
    <div>
                ...
                <ul class="mui-table-view ">
                    <!-- <li class="mui-table-view-cell ">
                        <a class="mui-navigate-right item_box">
                            ...
                            <input type="text" class="mui-text-center"  placeholder="请选择分组" readonly/>
                            <div class="text_3"></div>
                        </a>
                    </li> -->
                    <li class="mui-table-view-cell item_box">
                        <span class="text_1">商品名：</span>
                        <input type="text" class="mui-text-center" v-model="add_obj.name">
                        <div class="text_3"></div>
                    </li>
                    <li class="mui-table-view-cell">
        ...
        <Album v-show="Album_show" :size="4" :seetype="seetype" :type="seetype" v-on:setlist="reslist" />
...
export default {
    ...
    methods: {
        ...
        input_change(x) {
            ...
            if (!number_test.test(this.add_obj[x])) {
                this.add_obj[x] = '';
                return
            }
            ...
            if (x == "sellingPrice") {
                //售价
                if (this.add_obj.percentage && number_test.test(this.add_obj.percentage)) {
                    //比例
                ...
                if (this.add_obj.commissionPercentage && number_test.test(this.add_obj.commissionPercentage)) {                    this.add_obj.commissionPercentage = Math.floor(parseFloat(this.add_obj.commissionPercentage) * 10) / 10;
        ...
        add() {
            ...
            if (!this.add_obj.name) {
                ...
                if (this.add_obj.commission) {
                    if (this.add_obj.commission < 0 || this.add_obj.commission > 100) {
                        mui.toast("佣金填写有误", { duration: 2000, type: "div" });
                        return
                    } else if (this.add_obj.sellingPrice * this.招募信息.mincommissionscale / 100 > this.add_obj.commission) {
                        mui.toast("佣金不能小于" + this.add_obj.sellingPrice * this.招募信息.mincommissionscale / 100, { duration: 2000, type: "div" });
                        return
                    }
                }
                ...
                mui.toast("请输入专享省钱金额。", { duration: 2000, type: "div" });
                return;
                ...
                mui.toast("省钱金额不能大于商品金额的90%即" + this.add_obj.sellingPrice * 0.9 + "元。", { duration: "long", type: "div" });
                return;
            } else if (this.是否专享 && !this.专享商品.maxNumber) {
                mui.toast("请输入专享最多购买数量" + this.add_obj.sellingPrice * 0.9 + "元。", { duration: "long", type: "div" });
                return;
            } else if (this.是否专享 && (this.专享商品.maxNumber < 1 || this.专享商品.maxNumber > 100)) {
                mui.toast("专享购买数量为1-100" + this.add_obj.sellingPrice * 0.9 + "元。", { duration: "long", type: "div" });
                return
            }
        ...
        修改商品和修改专享() {
            openloading(true);
            Promise.all([this.修改商品([this.add_obj]), this.修改专享(this.专享商品)])
                .then(x => {
                    openloading(false);
        ...
        set_twtreid() {
            console.log(123);
            var obj = [{ value: 1, text: "%" }, { value: 2, text: "元" }];
            this.Picker1.setData(obj);
            this.Picker1.show(x => {
                console.log(x);
        ...
        获取专享商品() {
            this.loading = true;
            api_根据商品id查询专享(this.add_obj.id).then(x => {
                // 0 ? '新人专享': '生日专享';
                if (x.data.code == 200) {
        ...
        async 初始化() {
            ...
            api_查询招募列表(查询招募信息_query).then(x => {
                if (x.data.code == 200 && x.data.data.data.length > 0) {
                    this.招募信息 = x.data.data.data[0]
                }
            }).catch(err => { })
        }
    },
